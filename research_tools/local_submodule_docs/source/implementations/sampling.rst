...
get_av_uniqueness_from_triple_barrier function from the genieml_src package.
...
.. py:currentmodule:: Modules.sampling.concurrent
...
The core functions behind Sequential Bootstrapping are implemented in genieml_src and can be seen below:

.. py:currentmodule:: Modules.sampling.bootstrapping
...
One can use get_ind_matrix method from genieml_src to build indicator matrix from triple-barrier events.
...
To be as close to the genieml_src implementation let's convert ind_mat to numpy matrix
...
Let's see how this example is solved by the genieml_src implementation. To reproduce that:
...
genieml_src supports two methods of applying sample weights. The first is weighting an observation based on its given return
...
.. py:currentmodule:: Modules.sample_weights.attribution
