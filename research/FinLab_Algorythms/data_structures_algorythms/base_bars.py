...
def _prepare_columns_for_algo(
        # range-index with the date_time as a column, speeds up the algo (pd.to_datetime is slow)
        # same problem we had in genie using tick data
        df: pd.DataFrame,
        price_column_name: str = 'close'):
    # Check if any Datetime column exists in the column, e.g. DateTime, date_time, datetime,date time, etc.
    # This can be done by searching for the word 'time' and/or 'date' in the column names.
    # Assert only one column is found, else choose the first one.
    # If no column is found, then raise an error.
    if np.logical_or(df.columns.str.lower().str.contains('time'), df.columns.str.lower().str.contains('date')).any():
        orig_datetime_column_name = df.columns[
            np.logical_or(df.columns.str.lower().str.contains('time'), df.columns.str.lower().str.contains('date'))][0]
    else:
        raise ValueError('No column containing "time" or "date" was found in the dataframe')

    # Do the same for the price column, e.g. price, Price, PRICE, etc. Alternatively, you can also search for the word 'close'
    # in the column names. Assert only one column is found, else choose the first one.
    # If no column is found, then raise an error.
    if np.logical_or(df.columns.str.lower().str.contains(price_column_name),
                     df.columns.str.lower().str.contains('price')).any():
        orig_price_column_name = df.columns[np.logical_or(df.columns.str.lower().str.contains(price_column_name),
                                                          df.columns.str.lower().str.contains('price'))][0]
    else:
        raise ValueError('No column containing "price" was found in the dataframe')

    if df.columns.str.lower().str.contains('volume').any():
        orig_volume_column_name = df.columns[df.columns.str.lower().str.contains('volume')][0]
    else:
        raise ValueError('No column containing "volume" was found in the dataframe')

    df = df[[orig_datetime_column_name, orig_price_column_name, orig_volume_column_name]]
    df.columns = ["date_time", "price", "volume"]
    return df, [orig_datetime_column_name, orig_price_column_name, orig_volume_column_name]
...
class BaseBars(ABC):
        ...
        if isinstance(file_path_or_df, (list, tuple)):
            ...
            for file_path in file_path_or_df:
                for batch in pd.read_csv(file_path, chunksize=self.batch_size, parse_dates=[0],
                                         usecols=self.orig_column_names):
                    yield batch
            ...
            file_path_or_df, self.orig_column_names = _prepare_columns_for_algo(file_path_or_df)  # leaves only datetime, price, volume columns

            print(self.orig_column_names)
            print(file_path_or_df)
            for batch in _crop_data_frame_in_batches(file_path_or_df, self.batch_size):
                yield batch

        else:
            raise ValueError(
                'file_path_or_df is neither string(path to a csv file), iterable of strings, nor pd.DataFrame')
    ...
    def _read_first_row(self, file_path: str):
        ...
        first_row, self.orig_column_names = _prepare_columns_for_algo(first_row)
    ...
    def _extract_bars(self, data: Tuple[dict, pd.DataFrame]) -> list:
        ...
        for row in data:
            ...
            if (np.abs(self.thresholds['cum_theta']) > self.thresholds['exp_num_ticks'] * np.abs(
                    self.thresholds['expected_imbalance']) if ~np.isnan(
                self.thresholds['expected_imbalance']) else False):
