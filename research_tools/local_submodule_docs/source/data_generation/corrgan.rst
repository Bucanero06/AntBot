...
.. py:currentmodule:: Modules.data_generation.corrgan
...
.. note::
    ...
    Due to the CorrGAN trained models being too large to be included in the genieml_src package (> 100 MB).
    We included them as a downloadable package `here <https://drive.google.com/drive/folders/1-c4Z14a9JV56fvHQm-lB28vlaIjeLXVg>`_.
    Extract the ``corrgan_models`` folder and copy it in the ``genieml_src`` folder.
...
.. code-block::
    ...
    corr_mats = sample_from_corrgan(model_loc="genieml_src/corrgan_models", dim=100, n_samples=4)
