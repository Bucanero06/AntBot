...
def calculate_bet_size(events, predicted_probability, num_classes, predicted_side=None, discretization_step=0.0,
                       average_active_bets=False, num_threads=1):
    """
    Calculates the bet size using the predicted probability. Note that if 'average_active_bets' is True, the returned
    ...
    :param predicted_probability: (pandas.Series) The predicted probability.
    :param num_classes: (int) The number of predicted bet sides.
    :param predicted_side: (pd.Series) The predicted bet side. Default value is None which will return a relative bet size
     (i.e. without multiplying by the side).
    :param discretization_step: (float) The step size at which the bet size is discretized, default is 0.0 which imposes no
     discretization.
    :param average_active_bets: (bool) Option to average the size of active bets, default value is False.
    ...
    """
    initial_signal = get_signal(predicted_probability, num_classes, predicted_side)
    combined_data = initial_signal.to_frame('signal').join(events['t1'], how='left')

    if average_active_bets:
        averaged_signal = avg_active_signals(combined_data, num_threads)
    else:
        averaged_signal = combined_data.signal

    if abs(discretization_step) > 0:
        final_signal = discrete_signal(signal0=averaged_signal, step_size=abs(discretization_step))
    else:
        final_signal = averaged_signal
