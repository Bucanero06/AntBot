...
class Overview_StreamWidget:  # TODO: Will connect to the account streams for all exchanges, rn it's just okx
    ...
    async def get_all_exchanges_account_breakdown_table_card(self, box: str):
        ...
        return ui.stat_table_card(
            box=box,
            title=f'Live Account - {{exchange}}:messages@account - ${round(total_equity, 2)}',
    ...
    async def get_all_exchanges_live_positions_table_card(self, box: str):
        ...
        return ui.stat_table_card(
            box=box,
            title=f'Live Positions - {{exchange}}:messages@positions - {len(latest_positions_data)} positions',
    ...
    async def add_cards(self):
        ...
        await add_card(self.q, self.card_name + 'Accounts_Table',
                       await self.get_all_exchanges_account_breakdown_table_card(box='first_context_1'))
        await add_card(self.q, self.card_name + 'Positions_Table',
...
async def overview_page(q: Q):
    ...
    account_stream_widget = Overview_StreamWidget(q=q, card_name='Overview_Page_Account_Stream', count=2)
